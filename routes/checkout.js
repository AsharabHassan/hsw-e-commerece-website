...
async function checkoutLocals(req) {
  return { basket: await cart.hydrate(req.session) };
}
...
router.get('/checkout', async (req, res, next) => {
  try {
    ...
    res.render('shop/checkout.njk', {
      basket,
      values: { email: user?.email ?? '', ship_name: user?.name ?? '' },
      errors: {},
    });
  } catch (err) {
    next(err);
  ...
  async (req, res, next) => {
    try {
      ...
      if (!stripe.isConfigured()) {
        return res.status(503).render('shop/payments-unconfigured.njk', { basket });
      }
